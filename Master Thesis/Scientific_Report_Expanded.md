# SCIENTIFIC REPORT
**On the main results of the scientific qualification work (dissertation) for the degree of Candidate of Sciences (PhD)**
...
Historically, religious knowledge was guarded by physical lineages of scholars; today, in the digital epoch, we require "Algorithmic Guardians" that can protect the integrity of these lineages from the probabilistic distortions of generic Large Language Models. This dissertation formalizes the role of these guardians through the Agentix Islam framework, ensuring that the legacy of classical thought is transmitted with absolute precision across modern digital networks.
...
### 1.2 Degree of Development of the Research Topic
The application of Artificial Intelligence to education has been studied extensively over the past decade, but the existing body of work remains overwhelmingly concentrated on **analytical** rather than **generative authoritative** functions. A systematic mapping of the literature published between 2013 and 2023 (Scopus, IEEE Xplore, ACM Digital Library) identified five dominant lines of inquiry: sentiment-and-topic trio analysis, duo-technique forum mining, isolated keyword extraction, neural-network performance forecasting, and computer-vision engagement tracking. Each of these traditions excels at *evaluating* learners or *monitoring* the learning environment, but none addresses the central problem of producing **doctrinally faithful generative answers** in a specialized domain.

Within the narrower field of Retrieval-Augmented Generation (RAG), the work of Lewis et al. (2020), Gao et al. (2023), and Cohere’s multilingual reranking studies has established a strong methodological base for grounding LLM outputs in external corpora. However, the existing literature treats retrieval primarily as a *flat* semantic-similarity problem and does not address the **bibliographic geometry** of classical manuscripts (multi-volume hierarchies, nested chapters, page-anchored citations). In the specific domain of Islamic NLP, prior work has focused either on transliteration and morphology of Classical Arabic or on small, unstructured Hadith search prototypes; no published architecture combines (i) full *Sanad/Matn* separation, (ii) hierarchical Table-of-Contents indexing, and (iii) agentic orchestration with zero-hallucination guardrails.

This dissertation therefore occupies a clearly defined gap at the intersection of three under-developed strands: agentic RAG architectures, structured digitisation of classical Islamic texts, and empirically benchmarked reliability of LLMs in zero-tolerance theological reasoning.
...
### 1.4 Goal and Tasks of Research
...
1.  **Task 1 — Baseline NLP Linguistic Classification:** Creation of a programmatic NLP bridge to classify academic feedback. The focus was on moving beyond simple keyword matching toward custom Named Entity Recognition (NER). By training a model on scientific transitional words, the research established a "Technical Baseline" against which later generative reasoning could be compared (R1).
2.  **Task 2 — Systematic Literature Mapping (SLR):** A PRISMA-compliant review of a decade of educational AI research (2013–2023). By filtering over 1,000 papers, the goal was to identify the specific "Generative Void"—the missing capability for AI to act as an authoritative domain educator rather than just an analytical observer (R2).
3.  **Task 3 — Engineering the Ground-Truth Theological Corpus:** Structural digitization of the *Riyad al-Salihin* compilation. It involved the development of custom Python scripts to achieve the first complete separation of the narrator chain (*Sanad*) from the core prophetic text (*Matn*), creating a bilingual, semantic-ready Islamic dataset (R3).
4.  **Task 4 — Quantifying the Hallucination Gap:** An "LLM-as-a-Judge" benchmark testing state-of-the-art models (GPT-5, Gemini 2.5) against a gold-standard dataset of 10,000 Hanafi Fatwas, to mathematically prove the unreliability of native generative knowledge in theological contexts (R5).
5.  **Task 5 — Designing the Agentic RAG Architecture:** Implementation of the **Agentix Islam RAG** framework. This involved the development of TOC Hierarchy Mapping and Agentic Orchestration with strict scholarly guardrails, creating a zero-hallucination assistant for Islamic Jurisprudence (R6).

### 1.5 Scientific Novelty
The scientific novelty of this research lies in:
*   **Creation of the Ahadith Authenticity Dataset:** The first doctrinally clean schema for semantic Hadith processing that includes bilingual (Arabic–English) title mappings and structural separation of narrator chains from the core text.
*   **Methodology of TOC Hierarchy Mapping:** A novel architectural contribution to RAG systems in which the "geometry" of a classical book (multi-volume chapters, nested sub-sections, and contiguous page ranges) is flattened and stored as a JSON knowledge graph. This allows the agent to navigate classical corpora with the precision of a human librarian rather than performing simple flat-vector similarity.
...
*   **Multi-Stage Retrieval with Scholarly Guardrails:** The design of a unique two-stage retrieval pipeline (Similarity Search + Contextual Reranking) combined with "Mufti-Logic" system prompts that effectively terminate hallucination by enforcing a "declare-not-found" policy.

### 1.6 Theoretical and Practical Significance of the Work
*   **Theoretical Significance:** Extends the literature on Agentic RAG systems by demonstrating effective truncation of generative hallucinations in non-Western, zero-tolerance domains, and formalises **Bibliographic Geometry Mapping** as a first-class retrieval signal alongside semantic similarity.
*   **Practical Significance:** Deployment of a functional Web and Mobile Application (*Ahadeeth.ai*) directly aiding students and scholars globally, release of high-quality Arabic datasets on Kaggle for future research, and a scalable backend (Agentix Islam API) integrable into educational dashboards and chatbots.
...
*   **Systematic Evidence Mapping (PRISMA):** For the literature review (Chapter 2), we strictly adhered to the PRISMA (Preferred Reporting Items for Systematic Reviews and Meta-Analyses) protocol. This involved defining a multi-layered search string and executing it across Scopus, IEEE Xplore, and ACM Digital Library, followed by a rigorous three-pass filtering process (Title–Abstract, Full-Text, and Quality Appraisal).
...
### 1.8 Propositions to be Defended
...
*   **Statistical Rigor:** Calculation of Precision, Recall, and F1 metrics over a 500k-character NLP evaluation set.
*   **Live Deployment:** Stability of the architecture demonstrated over 4,000 real-world user interactions.
...
*   **KES AMSTA 2025** (The 19th International Conference on Agent and Multi-Agent Systems: Technologies and Applications):
    *   *Paper 1: "Engineering Ground Truth: The Ahadith AI Semantic Search Framework."* This session focused on the vector-database benchmarks and the *Sanad/Matn* extraction logic. Feedback from the multi-agent systems community highlighted the novelty of using reranking for theological prioritization.
...
### 1.10 Correspondence to the Passport of the Scientific Specialty
The results of this dissertation correspond to the items of the passport of scientific specialty **1.2.2 — "Mathematical modeling, numerical methods and software packages"**. Specifically:
...
*   Benchmarking of numerical methods (MMR, HNSW, contextual reranking) on large theological corpora.

### 1.11 Publication of Results and Intellectual Property
The research results are reflected in the following primary printed works, documenting the progression from early analytical NLP to advanced generative agents:
*   Two papers in Scopus-indexed conference proceedings (KES-AMSTA 2025).
*   One journal article in preparation on the *Fatawa AI* benchmark, currently being submitted to Scopus-indexed conferences.
*   Two technical reports registered with the University's Scientific Council.
*   **Intellectual Property:** A certificate of state registration is being prepared for the **Agentix Islam API** software architecture, covering the TOC hierarchy mapping code as a proprietary algorithmic contribution.

This satisfies the methodological requirement of at least two publications in peer-reviewed venues and active testing at international conferences.
...
#### 2.1.1 Technical Implementation and Methodology
...
#### 2.1.2 Gated NER and Semantic Segmentation
...
#### 2.1.3 Qualitative Findings of the NLP Phase
...
#### 2.1.4 Quantitative Evaluation and Metrics
...
#### 2.2.1 SLR Protocol and Search Methodology
The search process followed a strict multi-phase PRISMA protocol to ensure transparency and reproducibility. The search query was designed to be exhaustive, targeting three primary digital libraries: **Scopus, IEEE Xplore, and ACM Digital Library**.

#### 2.2.2 Rigorous Inclusion and Exclusion Criteria
...
*   **EC3 (Secondary):** Secondary literature (SLRs or Meta-analyses themselves) was excluded from the primary data synthesis but used to cross-verify the identified taxonomy.

#### 2.2.3 The Technological Taxonomy of Pre-Generative AI
...
#### 2.2.4 Synthesis and the Exposed "Generative Gap"
...
#### 2.3.1 Methodology for Dataset Creation
...
##### Structural Extraction Logic and Procedural Workflow
...
##### NLP Milestone: Bilingual Semantic Mapping
...
#### 2.3.2 Semantic Search and Vector Architecture
...
| :--- | :--- | :--- | :--- |
| **FAISS** | 1,350 ms | Local / In-Memory | HNSW / IVF |
| **Chroma** | 2,890 ms | Persistent Disk | Brute-force / HNSW |
| **Qdrant** | 3,620 ms | Cloud-Hosted | Segment-based |

##### Mathematical Logic: MMR vs Similarity
...
#### 2.3.3 Expert Theological Validation: The "Scholar-in-the-Loop" Benchmark
...
#### 2.4.1 The Fatawa AI Experiment: Quantifying the Hallucination Gap
...
##### The Hallucination Frequency Metric ($H_f$)
...
The theological implication was definitive: baseline LLMs hallucinate theological rulings in approximately **1 out of every 3 cases**, rendering the results fundamentally unauthoritative for scholarly religious education. This empirical failure provided the scientific justification for the transition to the Agentic RAG architecture.

#### 2.4.2 The Agentix Islam RAG Architecture
To eliminate these hallucinations, the **Agentix Islam Framework** was developed—a "knowledge-constrained reasoning engine."

##### Hierarchical TOC & JSON Schema
...
##### Contextual Compression and Rerank Depth
...
##### Agentic Orchestration and System Prompt Engineering
...
##### The Agentic Handshake and Orchestration Layer
The backend coordinates a complex "Agentic Handshake" between the user query and the underlying vector database. Unlike standard chatbots that pass a query directly to the LLM, the Agentix Islam framework follows a six-step deterministic protocol:
...
4.  **Level 4 — The Agentic Solution.** Chapter 4 effectively retired the "stochastic parrot" model of LLMs. By proving that hallucinations can be reduced to near zero through Hierarchical TOC Mapping and Mufti-Logic guardrails, the research established a new architectural standard for all high-sensitivity AI applications.
...
### 3.5 Prospects for Further Development of the Topic
...
## 4. LIST OF WORKS PUBLISHED BY THE AUTHOR ON THE DISSERTATION TOPIC
